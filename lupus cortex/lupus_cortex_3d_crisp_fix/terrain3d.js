...
const TerrainStudio = (() => {
  ...
  let structures = []; // user-placed markers/structures
  ...
  function reshapeGrid(elevations, resolution) {
    ...
    for (let r = 0; r < resolution; r++) {
      ...
      for (let c = 0; c < resolution; c++) {
        const v = elevations[r * resolution + c] ?? 0;
        row.push(v);
        if (v < min) min = v;
        if (v > max) max = v;
        sum += v;
  ...
  async function generate() {
    ...
    try {
      ...
      const { grid, min, max, mean } = await fetchElevationGrid(loc.lat, loc.lng, areaKm, resolution);
      ...
      terrainData = { center: loc, areaKm, resolution, grid, min, max, mean, imagery: satelliteImageryMeta };
      structures = [];
...
out geom;`;
    ...
    setStatus(`🏗️ Extruding ${buildings.length} buildings & ${roadWays.length} roads…`, 'busy');
    await new Promise((r) => setTimeout(r, 30)); // let the UI paint

    const builtBuildings = buildBuildingsMesh(buildings);
    ...
    updateStatsPanel();
    setStatus(`✅ GIS imported: ${builtBuildings} buildings · ${gisStats.roadsKm} km roads (OpenStreetMap).`, 'ok');
  }

  function buildingHeightMeters(tags) {
    let h = null;
    if (tags.height) { const v = parseFloat(tags.height); if (!isNaN(v)) h = v; }
    if (h === null && tags['building:levels']) { const l = parseFloat(tags['building:levels']); if (!isNaN(l)) h = l * DEFAULT_FLOOR_M; }
    if (h === null) h = DEFAULT_FLOOR_M * 3; // ~3 storeys default
    return Math.max(3, Math.min(400, h)); // clamp bad OSM tags
  }
  ...
  function buildBuildingsMesh(ways) {
    ...
    for (const el of ways) {
      ...
      const hMeters = buildingHeightMeters(el.tags || {});
      ...
      const record = {
        type: 'building', source: 'gis', mesh,
        nx: cx / PLANE_SIZE, nz: cz / PLANE_SIZE,
        heightUnits: hUnits, heightMeters: hMeters, sizeScale: 1,
      };
      mesh.userData = { heightUnits: hUnits, heightMeters: hMeters, baseColor: mat.color.getHex(), record };
      structureGroup.add(mesh);
      structures.push(record);
      if (++count >= BUILDING_EDIT_CAP) break; // keep it editable & GPU-friendly
    }
    return count;
  }
  ...
  function buildExportData() {
    ...
    return {
      meta: {
        ...
        edited: sculptCount > 0 || structures.length > 0,
      ...
      changes: {
        ...
        plannedRoads: roads.length,
        zonedCells: zoneGrid ? zoneGrid.filter((zone) => zone !== 'none').length : 0,
      },
  ...
  // ---------------------------------------------------------------------------
  // Send design data to Gemini for analysis
  // ---------------------------------------------------------------------------
  async function analyzeWithGemini() {
    ...
    if (body) body.innerHTML = '<div class="loading"></div><p>Analyzing terrain with Gemini…</p>';
...
Keep it structured with short headers and bullet points.`;
  ...
  function bindControls() {
    ...
    document.getElementById('terrainAnalyzeBtn')?.addEventListener('click', analyzeWithGemini);
